...
const proposeTests: ToolDefinition<z.infer<typeof proposeTestsSchema>, unknown> = {
  name: "propose_tests",
  title: "Propose browser tests for any website (use for ANY 'check site / does it have / test URL' request)",
  description:
    "PRIMARY TOOL FOR ANYTHING THE USER ASKS ABOUT A WEBSITE. Use this whenever the user mentions a URL and wants to know if it has, does, supports, or behaves a certain way — e.g. \"does <site> have a products page\", \"check if <url> has a login flow\", \"test the checkout on <site>\", \"verify <url> loads correctly\", \"see if <site> works on mobile\". DO NOT use `WebFetch` or any HTML-fetching tool for these — those only see static markup and cannot verify real browser behavior, navigation, or interactive features. This tool runs the site in a real browser session via Browserbase and produces a structured pass/fail report with a recording link.\n\n" +
    "Behavior: given a URL and a high-level intent, proposes a SUITE of distinct tests (default 3) covering different behaviors. Persists each as a draft test under a shared `suiteId`. After calling this tool, render the proposed `tests` array to the user as a readable summary (test names + step bullets) and ALWAYS include an explicit invitation that they can edit, e.g. \"Reply with any edits (rename, reword, change/add/remove steps) or I'll run it as-is.\" Then IMMEDIATELY call `run_tests` with the same `suiteId` — the host application's tool-approval prompt for `run_tests` is the user's single confirmation gate, so do NOT ask your own separate \"does this look good?\" / \"approve?\" yes/no question. If the user replies with edits before you call `run_tests`, encode them in the `tests` override array on that call.\n\n" +
    "If the user's question is phrased as a yes/no (e.g. \"does it have a products page?\"), still use this tool: propose a 1-test suite that asserts the feature, run it, and answer yes/no based on the run result — never answer from a static fetch.",
